...
## Approved Bitcoin and Kraken decisions

### 11. Bitcoin and Kraken capability scope

Bitcoin and Kraken treasury operations are a primary HVE-CFO capability. The
scope includes Bitcoin systems, custody, on-chain operations, Lightning,
market structure, Kraken data, treasury accounting, risk analysis, and cited
research.

HVE-CFO remains advisory/controller-only. It must not execute trades, withdraw
funds, or make final treasury decisions.

### 12. Read-only Kraken integration

The initial Kraken integration is read-only and may cover balances, asset
ledgers, fills, order history, deposits, withdrawals, fees, exports, and
API-health checks.

The integration must technically prohibit order placement or cancellation,
withdrawals, transfers, API-key changes, account-security changes, and
automatic rebalancing. Any future execution path requires a separate approved
decision and authorized owner.

### 13. Kraken data and credential handling

The CFO will use a dedicated read-only Kraken credential or equivalent
export-only access. Credentials must not be stored in profile files and must be
supplied only through the host's approved secret-management mechanism.

The CFO may ingest only account and asset-ledger data necessary for treasury
reconciliation and reporting. Account identifiers and other sensitive values
must remain protected.

### 14. Bitcoin knowledge corpus

The local, provenance-backed knowledge corpus uses this source order:

1. Bitcoin Core, BIPs, and official technical documentation.
2. Official Kraken and network-service documentation.
3. CRA/Ontario and accountant-approved tax material.
4. Approved HVE policy and treasury records.
5. Attributed public research representing technical, monetary, payments, and
   operational viewpoints.

Each source must retain its URL or document ID, retrieval date,
jurisdiction/network context, freshness, and citation status. Copyrighted works
are summarized and linked, not reproduced.

### 15. Bitcoin tax and accounting boundary

HVE-CFO may classify Bitcoin transactions, produce CAD/SAT reporting, track
fees, prepare cost-basis inputs, reconcile account activity, and draft tax
workpapers.

Deterministic tools own all calculations. Hans and a qualified Canadian
accountant retain authority over tax treatment, filing positions, remittances,
elections, and returns. The model must not invent tax payable or present an
uncited tax conclusion as definitive.

### 16. Trading-risk language

The legacy 1% maximum-risk-per-trade rule, 0.26% Kraken taker-fee assumption,
2% daily and 5% weekly drawdown limits, and paper-trading language are retained
as analysis-only scenario controls and calculation defaults.

They do not grant execution authority and are not active company policy. Any
live trading limit, fee assumption, or execution policy requires a separate
dated Hans decision.

### 17. Bitcoin and Kraken readiness gate

Before the Bitcoin/Kraken capability is declared production-ready, evidence
must include:

- held-out Kraken-export reconciliation;
- balance, fee, deposit, and withdrawal matching;
- SAT/CAD correctness;
- source-citation and freshness tests;
- custody and API-permission risk detection;
- refusal tests for trades and withdrawals;
- deterministic-number verification;
- degraded-service behavior;
- human-scored treasury reports.

This gate does not authorize profile or runtime changes by itself.
...
- No Kraken trading, withdrawal, transfer, or account-security operation is
  authorized.
- No Bitcoin or Kraken knowledge claim is authoritative without provenance and
  freshness metadata.
...
- Live read-only systemd, Ollama, MCP, and Open WebUI health evidence on
  2026-09-02
- Seven official Hans decisions recorded during the 2026-09-02 CFO Bitcoin and
  Kraken scope review
