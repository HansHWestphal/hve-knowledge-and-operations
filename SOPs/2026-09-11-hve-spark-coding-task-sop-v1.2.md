...
**Effective:** Pending Hans approval
**Supersedes:** v1.1 upon adoption
...
### 6.1 Controlled local-agent pilot

A local-agent pilot is a bounded experiment, not an open-ended batch. Before
the first task is submitted, Luna must record the cohort ID, task count, task
classes, operator, approved workspace root, rollback artifact, per-task
timeout, queue-claim deadline, no-progress deadline, and absolute cohort
deadline.

The pilot runner must enforce all of the following:

- one active worker job unless an explicit concurrency decision is recorded;
- fresh isolated workspaces with a verified effective tool workspace;
- `READY_FOR_DELEGATION` preflight for every task;
- a queue-claim and worker-start deadline;
- a per-task execution timeout shorter than the cohort deadline;
- bounded polling and a no-progress watchdog based on heartbeat/progress;
- stop-on-first-failure unless a different continuation policy is approved;
- immediate abort on workspace divergence, unexpected worker exit, or
  incomplete telemetry;
- cancellation through the supported queue API with an explicit reason;
- a final queue, worker, workspace, and metrics snapshot after stopping.

For the Phase 6 pilot class, the default cohort deadline is **15 minutes**.
The runner must report progress at least once per minute. Reaching the
deadline is a controlled stop, not a success condition. A pilot may pass only
when every task has terminal status, complete telemetry, safety PASS, review
PASS, and an evaluation meeting the approved thresholds.
...
For a pilot, Luna must independently verify the cohort deadline, task count,
claim/start timing, no-progress behavior, worker health, workspace outputs,
terminal queue states, telemetry completeness, and evaluation evidence. A
missing field is a blocked evidence gate; it must never be inferred as success.
...
- preserve the pilot rollback artifact and all failed or stopped workspaces
  until the evidence record is complete;
...
### 10.1 Pilot evidence minimum

The pilot record must include the cohort ID, task IDs, start and stop times,
absolute deadline, per-task queue and worker transitions, heartbeat/progress
evidence, workspace paths, model, token counts, turns, tool calls, telemetry
status, safety and review results, wall-clock duration, intervention and
recovery counts, cancellation reasons, rollback artifact, and final decision.
The record must state whether the cohort was stopped by completion, failure,
watchdog, operator request, or deadline.
...
This v1.2 revision is adopted by Hans Westphal effective 2026-09-11 and
supersedes v1.1. Changes to authority, approval gates, repository boundaries,
